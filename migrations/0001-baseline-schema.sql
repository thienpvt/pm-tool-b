...
--
-- Part 1 below is the 35 CREATE TABLE statements that used to live in
-- initPostgresSchema(). Part 2 is the cumulative ALTER/CREATE set that used to
-- live in the migratePostgresSchema() array, appended verbatim and in its
-- original order.
--
-- Part 2 is deliberately NOT folded into the Part 1 table bodies. Many of its
-- statements are no-ops against a database Part 1 just built, but they are the
-- only repair path for a PRE-EXISTING database whose columns the old loop may
-- never have added -- that loop swallowed every error, so which statements
-- actually succeeded is unknown. CREATE TABLE IF NOT EXISTS cannot add a column
-- to a table that already exists; ALTER TABLE ... ADD COLUMN IF NOT EXISTS can.
-- Keep both parts.
--
-- Future schema changes get their own NNNN-*.sql file; do not edit this one
-- (the ledger stores its sha256 and the runner refuses a drifted checksum).

-- =============================================================================
-- Part 1: baseline tables (from initPostgresSchema)
-- =============================================================================
...
-- =============================================================================
-- Part 2: cumulative schema additions (from the migratePostgresSchema array)
-- Idempotent by construction; the two ALTER COLUMN TYPE conversions are
-- DO-guarded on the column's current type.
-- =============================================================================

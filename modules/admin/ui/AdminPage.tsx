...
export default function AdminPage() {
  ...
  // ΓöÇΓöÇ Companies ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Jira Config ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  const saveJiraConfig = async () => {
    ...
    if (!res.ok) { toast.error('L╞░u thß║Ñt bß║íi'); return; }
    toast.success(`─É├ú l╞░u cß║Ñu h├¼nh Jira cho ${jiraCompany.name}`);
    setJiraOpen(false);
  };
  ...
  const testJiraConfig = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        setJiraTestResult({ ok: true, message: `Kß║┐t nß╗æi th├ánh c├┤ng ΓÇö ${data.displayName} (${data.email})` });
      } else {
        setJiraTestResult({ ok: false, message: data.error ?? 'Lß╗ùi kh├┤ng x├íc ─æß╗ïnh' });
      }
    } catch {
      setJiraTestResult({ ok: false, message: 'Kh├┤ng kß║┐t nß╗æi ─æ╞░ß╗úc tß╗¢i server' });
    } finally {
      setJiraTesting(false);
    }
  };

  // ΓöÇΓöÇ RAG Config ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  const saveRagConfig = async () => {
    ...
    if (!res.ok) { toast.error('L╞░u thß║Ñt bß║íi'); return; }
    toast.success(`─É├ú l╞░u cß║Ñu h├¼nh RAG cho ${ragCompany.name}`);
    setRagOpen(false);
  };

  // ΓöÇΓöÇ Users ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Demo Requests ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen">
      ...
      <main className="flex-1 p-4 lg:p-6">
        ...
        {/* ΓöÇΓöÇ Users tab ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        {tab === 'users' && (
            ...
            <div className="flex flex-wrap items-center gap-2 px-5 py-3 border-b bg-slate-50/50">
              <div className="relative flex-1 min-w-[180px] max-w-xs">
                ...
                <Input
                  ...
                  placeholder="Search username, email, nameΓÇª"
                  value={searchQ}
              ...
              <tbody className="divide-y">
                {users.map(u => (
                  <tr key={u.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3 text-slate-600">{u.display_name || 'ΓÇö'}</td>
                    <td className="px-5 py-3 text-slate-600 text-xs">{u.email || 'ΓÇö'}</td>
        ...
        {/* ΓöÇΓöÇ Companies tab ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
                  ...
                  <tr key={c.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => openJiraConfig(c)} className="text-slate-400 hover:text-violet-600 transition-colors" title="Cß║Ñu h├¼nh Jira">
                          <Link2 className="h-4 w-4" />
                        </button>
                        <button onClick={() => openRagConfig(c)} className="text-slate-400 hover:text-emerald-600 transition-colors" title="Cß║Ñu h├¼nh RAG">
        ...
        {/* ΓöÇΓöÇ Requests tab ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        ...
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Notes ΓÇö {noteRequest?.full_name}
            </DialogTitle>
          </DialogHeader>
          <div className="py-2">
            <Label>Note</Label>
      ...
      {/* ΓöÇΓöÇ Company Dialog ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Jira Config Dialog ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <Dialog open={jiraOpen} onOpenChange={setJiraOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Link2 className="h-4 w-4 text-violet-600" />
              Cß║Ñu h├¼nh Jira ΓÇö {jiraCompany?.name}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <p className="text-xs text-slate-500 bg-slate-50 rounded-lg p-3 leading-relaxed">
              Nhß║¡p <strong>t├¬n biß║┐n m├┤i tr╞░ß╗¥ng</strong> ─æ├ú set tr├¬n server cho c├┤ng ty n├áy.
              V├¡ dß╗Ñ: nß║┐u bß║ín set <code className="bg-slate-200 px-1 rounded">JIRA_URL_ACME=https://acme.atlassian.net</code> th├¼ nhß║¡p <code className="bg-slate-200 px-1 rounded">JIRA_URL_ACME</code> v├áo ├┤ d╞░ß╗¢i.
            </p>
            <div>
              <Label>T├¬n biß║┐n Base URL <span className="text-red-500">*</span></Label>
              <Input
              ...
              />
              <p className="text-xs text-slate-400 mt-1">Gi├í trß╗ï trß╗Å tß╗¢i: <code>https://yourcompany.atlassian.net</code></p>
            </div>
            <div>
              <Label>T├¬n biß║┐n Email <span className="text-red-500">*</span></Label>
              <Input
                className="mt-1.5 font-mono"
              ...
              />
              <p className="text-xs text-slate-400 mt-1">Gi├í trß╗ï l├á email ─æ─âng nhß║¡p Jira</p>
            </div>
            <div>
              <Label>T├¬n biß║┐n API Token <span className="text-red-500">*</span></Label>
              <Input
                className="mt-1.5 font-mono"
              ...
              />
              <p className="text-xs text-slate-400 mt-1">Lß║Ñy token tß║íi: id.atlassian.com ΓåÆ Security ΓåÆ API tokens</p>
            </div>
          ...
          <DialogFooter className="flex justify-between items-center">
            <Button variant="outline" onClick={testJiraConfig} disabled={jiraTesting}
              ...
              {jiraTesting
                ? <><Loader2 className="h-3.5 w-3.5 animate-spin" /> ─Éang kiß╗âm tra...</>
                : <><Link2 className="h-3.5 w-3.5" /> Kiß╗âm tra kß║┐t nß╗æi</>}
            </Button>
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setJiraOpen(false)}>Huß╗╖</Button>
      ...
      {/* ΓöÇΓöÇ RAG Config Dialog ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <Dialog open={ragOpen} onOpenChange={setRagOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Gauge className="h-4 w-4 text-emerald-600" />
              Cß║Ñu h├¼nh RAG ΓÇö {ragCompany?.name}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <p className="text-xs text-slate-500 bg-slate-50 rounded-lg p-3 leading-relaxed">
              ─Éiß╗üu chß╗ënh ng╞░ß╗íng t├¡nh chß╗ë sß╗æ sß╗⌐c khß╗Åe RAG cho c├┤ng ty n├áy. <strong>SPI</strong> (Schedule Performance Index) = tiß║┐n ─æß╗Ö thß╗▒c tß║┐ / tiß║┐n ─æß╗Ö kß╗│ vß╗ìng theo thß╗¥i gian.
            </p>
            <div className="grid grid-cols-2 gap-3">
              {([
                { key: 'spi_red_threshold',   label: 'SPI ΓåÆ RED',           hint: 'VD: 0.6 = tiß║┐n ─æß╗Ö < 60% kß╗│ vß╗ìng', step: '0.05', min: '0.1', max: '0.99' },
                { key: 'spi_amber_threshold', label: 'SPI ΓåÆ AMBER',         hint: 'VD: 0.8 = tiß║┐n ─æß╗Ö < 80% kß╗│ vß╗ìng', step: '0.05', min: '0.1', max: '0.99' },
                { key: 'deadline_red_days',   label: 'Deadline RED (ng├áy)',  hint: '0 = ─æ├ú qu├í hß║ín',                  step: '1',    min: '-365', max: '30' },
                { key: 'deadline_amber_days', label: 'Deadline AMBER (ng├áy)',hint: 'C├▓n Γëñ X ng├áy th├¼ AMBER',          step: '1',    min: '1',   max: '90' },
                { key: 'risks_red',           label: 'Risks ΓåÆ RED',         hint: 'ΓëÑ X risk mß╗ƒ',                     step: '1',    min: '1',   max: '20' },
                { key: 'risks_amber',         label: 'Risks ΓåÆ AMBER',       hint: 'ΓëÑ X risk mß╗ƒ',                     step: '1',    min: '1',   max: '10' },
                { key: 'issues_amber',        label: 'Issues ΓåÆ AMBER',      hint: 'ΓëÑ X issue mß╗ƒ',                    step: '1',    min: '1',   max: '10' },
                { key: 'low_progress_amber',  label: 'Tiß║┐n ─æß╗Ö thß║Ñp ΓåÆ AMBER',hint: '< X% khi SPI kh├┤ng t├¡nh ─æ╞░ß╗úc',    step: '5',    min: '0',   max: '80' },
          ...
          <DialogFooter className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setRagOpen(false)}>Huß╗╖</Button>
              <Button onClick={saveRagConfig} className="bg-emerald-600 hover:bg-emerald-700">L╞░u cß║Ñu h├¼nh</Button>
            </div>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* ΓöÇΓöÇ Reset Password Dialog ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <Dialog open={resetOpen} onOpenChange={setResetOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <KeyRound className="h-4 w-4 text-amber-500" />
              Reset Password ΓÇö {resetUser?.username}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div>
      ...
      {/* ΓöÇΓöÇ User Dialog ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <Dialog open={userOpen} onOpenChange={setUserOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editUser ? `Edit User ΓÇö ${editUser.username}` : 'Add User'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            {!editUser && (
              <div>

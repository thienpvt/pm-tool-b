...
export async function exportWeeklyReportHandler(
  _req: NextRequest,
  { body }: HandlerContext<{ id: string }> & { project: ProjectAccessRow },
) {
...
async function buildWeeklyReportResponse(
  ...
  if (reportData.openRisks.length > 0 || reportData.openIssues.length > 0) {
    ...
    r2++;
    for (const risk of reportData.openRisks) {
      const row = ws2.getRow(r2++);
      row.getCell(1).value = 'Risk';
      row.getCell(2).value = risk.priority;
      row.getCell(3).value = risk.description;
      row.getCell(4).value = risk.mitigation || '';
      [1, 2, 3, 4].forEach(c => { row.getCell(c).font = { size: 9 }; });
    }
    for (const issue of reportData.openIssues) {
      const row = ws2.getRow(r2++);
      row.getCell(1).value = 'Issue';
      row.getCell(2).value = issue.priority;
      row.getCell(3).value = issue.description;
      row.getCell(4).value = issue.mitigation || '';
      [1, 2, 3, 4].forEach(c => { row.getCell(c).font = { size: 9 }; });
    }
  }

...
import {
  ...
  listIssues,
  updateIssue,
} from '@/modules/projects/backend/services/issues.service';
...
export async function postIssuesHandler(
  ...
  return NextResponse.json(
    await createIssue(params.id, actor, body as Record<string, unknown>),
    { status: 201 },
  );
}
...
export async function putIssuesHandler(
  ...
  return NextResponse.json(
    await updateIssue(params.id, actor, rowId as string | number, fields),
  );
}

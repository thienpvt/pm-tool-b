...
const { projectAccessRow, listJiraKeys } = vi.hoisted(() => ({
...
describe('GET /api/projects/[id]/activities/import', () => {
  beforeEach(() => vi.clearAllMocks());
  const params = { params: Promise.resolve({ id: '7' }) };
  const req = () => new NextRequest('http://localhost/api/projects/7/activities/import');
  ...
  it('returns 403 for a cross-company project', async () => {
    ...
    expect(listJiraKeys).not.toHaveBeenCalled();
  });

  it('returns jira keys for owner', async () => {
    vi.mocked(getSessionFromRequest).mockResolvedValue(owner as never);
    projectAccessRow.mockResolvedValue({ company_id: 5, customer_company_id: null });
    listJiraKeys.mockResolvedValue(['EPIC-1']);
    const res = await GET(req(), params);
    expect(res.status).toBe(200);
    await expect(res.json()).resolves.toEqual(['EPIC-1']);
  });
});

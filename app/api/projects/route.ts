  ...
  try {
    ...
    const projects = user.is_admin
      ...
      : user.company_id !== null
        ? await db.all(`SELECT p.*, c.name as program_name, c.industry as program_industry FROM projects p LEFT JOIN customers c ON p.customer_id = c.id WHERE (p.company_id = ? OR c.company_id = ?) ORDER BY p.created_at DESC`, user.company_id, user.company_id)

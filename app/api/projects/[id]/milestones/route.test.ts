...
const { projectAccessRow, listMilestonesRepo } = vi.hoisted(() => ({
...
describe('GET /api/projects/[id]/milestones', () => {
  ...
  const req = () => new NextRequest('http://localhost/api/projects/7/milestones');
  ...
  it('returns 200 list for owner', async () => {
    ...
    projectAccessRow.mockResolvedValue({ company_id: 5, customer_company_id: null });
    listMilestonesRepo.mockResolvedValue([{ id: 1, name: 'M1' }]);
    const res = await GET(req(), params);
    expect(res.status).toBe(200);
    await expect(res.json()).resolves.toEqual([{ id: 1, name: 'M1' }]);
  });
});

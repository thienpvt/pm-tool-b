...
import { Upload, Bug, CheckCircle2, Clock, RefreshCw, ChevronLeft, ChevronRight, FlaskConical, Sparkles } from 'lucide-react';
...
export default function BugsPage() {
  ...
  const statusChartData = useMemo(() => {
    ...
    for (const b of bugs) {
      ...
      if (['To Do', 'In Progress', 'Reopen'].includes(s)) {
  ...
  const newTodayCount = useMemo(() => bugs.filter(b => {
    if (!b.created) return false;
    const d = b.created.substring(0, 10);
    return d === today;
  }).length, [bugs, today]);
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {bugs.length > 0 ? `${bugs.length} bug đã import` : 'Chưa có dữ liệu — Import từ Jira CSV'}
                </p>
              </div>
            </div>
          ...
          {!loading && bugs.length > 0 && activeTab === 'summary' && (
            <div className="space-y-6">
              ...
              {/* Row 1: Active bugs table + 2 pie charts */}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-xl border border-amber-200 shadow-sm p-5 flex items-center gap-4">
                  ...
                  {newTodayCount > 0 && (
                    <div className="ml-auto">
                      <Badge className="bg-amber-500 text-white border-0">Mới</Badge>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
                <p className="text-xs text-slate-400 uppercase tracking-wide font-medium mb-3">Tổng quan</p>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        <th className="text-left px-3 py-3 font-semibold text-slate-600 text-xs whitespace-nowrap">Issue Type</th>
                        <th className="text-left px-3 py-3 font-semibold text-slate-600 text-xs whitespace-nowrap">Key</th>
                        <th className="text-left px-3 py-3 font-semibold text-slate-600 text-xs whitespace-nowrap">ID</th>
                        <th className="text-left px-3 py-3 font-semibold text-slate-600 text-xs whitespace-nowrap min-w-[200px]">Summary</th>
                        <th className="text-left px-3 py-3 font-semibold text-slate-600 text-xs whitespace-nowrap">Assignee</th>
                        <th className="text-left px-3 py-3 font-semibold text-slate-600 text-xs whitespace-nowrap">Reporter</th>
                        <th className="text-left px-3 py-3 font-semibold text-slate-600 text-xs whitespace-nowrap">Priority</th>
                        <th className="text-left px-3 py-3 font-semibold text-slate-600 text-xs whitespace-nowrap">Status</th>
                        <th className="text-left px-3 py-3 font-semibold text-slate-600 text-xs whitespace-nowrap">Resolution</th>
                        <th className="text-left px-3 py-3 font-semibold text-slate-600 text-xs whitespace-nowrap">Created</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {pagedBugs.length === 0 ? (
                        ...
                        <tr key={bug.id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-3 py-2.5">
                            {bug.priority ? (
                              <span className={`text-xs px-1.5 py-0.5 rounded border font-medium ${PRIORITY_BADGE[bug.priority] ?? 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                                {bug.priority}
                              </span>
                            ) : <span className="text-slate-300">—</span>}
                          </td>
                          <td className="px-3 py-2.5">
                            {bug.status ? (
                              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_BADGE[bug.status] ?? 'bg-slate-100 text-slate-600'}`}>
                                {bug.status}
                              </span>
                            ) : <span className="text-slate-300">—</span>}
                          </td>
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-slate-50">
                    <p className="text-xs text-slate-500">
                      Trang {page}/{totalPages} · {filteredBugs.length} bug
                    </p>

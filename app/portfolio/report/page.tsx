...
export default function PortfolioReportPage() {
  ...
  const openEmailModal = () => {
    ...
    setSavePromptName('');
    const tpl = EMAIL_PROMPT_TEMPLATES.find(t => t.id === selectedPromptId) ?? EMAIL_PROMPT_TEMPLATES[0];
    if (!customPromptText) setCustomPromptText(tpl.instruction);
  ...
  const generateEmailContent = async () => {
    ...
    try {
      ...
      const instruction = customPromptText.trim() || template.instruction;
  ...
  const savePrompt = () => {
    if (!savePromptName.trim() || !customPromptText.trim()) return;
    if (savedPrompts.length >= MAX_SAVED_PROMPTS) { toast.error(`Tối đa ${MAX_SAVED_PROMPTS} prompt đã lưu`); return; }
    const newPrompt: SavedPrompt = { id: Date.now().toString(), name: savePromptName.trim(), text: customPromptText };

...
describe.skipIf(!hasTestDb)('runMigrations against real Postgres', () => {
  ...
  it('applies the shipped baseline once and is a no-op on the second run', async () => {
    const client = await testPool().connect();
    try {
      const baselineSql = readFileSync(
        fileURLToPath(new URL('../../migrations/0001-baseline-schema.sql', import.meta.url)),
        'utf8',
      );
      const baselineFile = parseMigrationFile('0001-baseline-schema.sql', baselineSql);

      const first = await runMigrations(client, [baselineFile], { ledgerTable: LEDGER });
      expect(first.applied).toEqual(['0001-baseline-schema.sql']);

      const ledger = await client.query(`SELECT version, name, checksum FROM ${LEDGER}`);
      expect(ledger.rows).toHaveLength(1);
      expect(ledger.rows[0]).toMatchObject({ version: 1, name: '0001-baseline-schema.sql' });

      for (const t of ['companies', 'users', 'projects']) {
        const res = await client.query(`SELECT to_regclass('public.${t}') AS cls`);
        expect(res.rows[0].cls).not.toBeNull();
      }

      const second = await runMigrations(client, [baselineFile], { ledgerTable: LEDGER });
      expect(second.applied).toEqual([]);
      expect(second.alreadyApplied).toEqual(['0001-baseline-schema.sql']);

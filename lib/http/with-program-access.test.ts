...
describe('withProgramAccess', () => {
  ...
  describe('ACCESS_ENFORCEMENT shadow flag', () => {
    ...
    it('shadow ON: a cross-company deny still invokes the handler, with program undefined', async () => {
      ...
      const [, ctx] = handler.mock.calls[0];
      expect(ctx.program).toBeUndefined();
      expect(assertProgramAccess).toHaveBeenCalledTimes(1);
    ...
    it('shadow ON: an unrelated handler error (post-assert) is NOT softened, still 500', async () => {
      process.env.ACCESS_ENFORCEMENT = 'shadow';
      getSessionFromRequest.mockResolvedValue(ownerSession);
      assertProgramAccess.mockResolvedValue({ id: 7, company_id: 5 });

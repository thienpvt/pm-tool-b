...
| Date | Change |
|---|---|
| 2026-06-22 | Portfolio Report FTE Overhead fix — the "Cơ cấu năng lực khối" donut/pie computed `overheadRemainingFte` from the raw `overhead_remaining` column, so overhead members (who normally leave it unset) contributed ~0%, making the Overhead slice incorrectly tiny. `app/api/portfolio/report/route.ts` now applies the canonical `overheadRemainingOf` model used by `/resources` + `/portfolio/resources`: per overhead member, out-of-project overhead = explicit `overhead_remaining` if >0 else `max(0, 1 − current_month_fte)` (an overhead person = 1.0 FTE overhead by default). Email-generated report inherits the fix via `fteStats`. |
| 2026-06-20 | Portfolio Roadmap — milestone view + expandable epics + epic detail: (1) New view-mode toggle in header — **Theo Phase** (default Gantt) / **Theo Milestone**. (2) Phase mode: each phase bar is now a button that toggles expansion to reveal its Epics as dashed sub-bars on the same timeline canvas (positioned by epic plan dates, fallback phase range; fill = weighted-by-status %). Epics lazy-loaded per project into `epicsByProject` state. Clicking an epic bar opens the shared Epic detail dialog. (3) Milestone mode: `GET /api/portfolio/milestones` lists all milestones across visible projects (grouped by project in `<optgroup>`); selecting one fetches `/api/projects/[id]/milestones/[mid]/epics` and renders epics→children grouped by phase with weighted progress (same `itemPct`/`weightedProgress` logic as the milestones page), overall weighted bar in header. (4) Epic detail dialog (used by both modes): children table (Key/No, Activity, Status, %, start/end) + overall weighted progress bar across all children at top, via `weightedProgress(children.statuses)`. New API `GET /api/portfolio/roadmap/epics?project_id=` returns epic→children tree with `weighted_pct` (computed server-side from `lib/status-weights`). |
| 2026-06-20 | Resource Management fixes — peak-load double count & overhead FTE: (1) `/resources` Global Resource View "Total Load (peak)" was keyed by `name.toLowerCase()`, so two different people sharing a name were merged and double-counted. Now keyed by email (fallback name) via new `personKey()` helper — applied to `personMap`, summary `allPersonMap`/`totalPersons`, and unassigned-internal matcher (matches by email or name). (2) `GET /api/portfolio/members` `current_month_fte` + `program_count` subqueries matched `team_members` by name only — now match by email when `portfolio_members.email` is set, else by name, fixing the same conflation at the FTE source. (3) FTE Overhead KPI showed ~0–1% for many overhead members because overhead FTE = `current_month_fte + overhead_remaining` and overhead people are usually not in projects (fte≈0) with `overhead_remaining` left unset (0). New model: each overhead member = 1.0 FTE overhead by default — `overheadRemainingOf(m)` returns explicit `overhead_remaining` if >0 else `max(0, 1 − current_month_fte)`; `overheadFteOf(m)` = in-project + remaining. Applied in both `/resources` ("FTE Overhead" card) and `/portfolio/resources` ("FTE Overhead còn lại" card). |
| 2026-06-20 | Milestone weighted progress: (1) New `lib/status-weights.ts` — single source of truth for `STATUS_WEIGHTS`, `statusWeight`, `statusPct` (weight×100), `DONE_STATUSES`, `weightedProgress`. `app/api/portfolio/report/route.ts` refactored to import from it (removed local dup). (2) Milestones page (`/projects/[id]/milestones`) now computes % from status weight instead of stored `completion_pct`: each story/task % = `statusPct(status)` (Done=100, In Testing=60, In Progress=30, ...), EPIC % = average of its children's %, milestone overall = Σ(weights)/leaf-count (EPICs excluded). New overall weighted-progress bar in header + PDF export meta; picker dialog and PDF rows also show weighted %. (3) One-time guarded DB backfill `backfillWeightedCompletion()` in `lib/db.ts` (flag `completion_pct_weighted_v1` in `settings`) — updates existing `activities.completion_pct`: leaves from status weight, EPICs from child average. |
| 2026-06-11 | Bug Leaders dashboard + PDF Export: (1) New API `GET /api/portfolio/bug-assignees` — returns bug count per assignee (total, active, critical, projects list) aggregated from latest snapshot per project, filtered by company; (2) Dashboard (`app/page.tsx`): new "Bug Leaders" section at bottom — input threshold X (default 5), table lists assignees with X+ bugs across all projects, Export PDF button uses `jspdf`+`html-to-image` to capture section; (3) Bug page per project (`/projects/[id]/bugs`): Export PDF button in header captures current tab (Summary or List) as multi-page PDF. |
| 2026-06-10 | Portfolio Report: (1) Fix Epic counting — activities now grouped by parent EPIC's name (`no='EPIC'` + `parent_id`) instead of `phase`, restoring correct Epic count per project after timeline refactor; (2) Bug Report section added to both HTML and text reports — positioned before Resource Allocation; shows total bugs KPI cards, status donut chart, priority donut chart, and stacked bar chart by project (bars divided by status). `bugStats` data added to `GET /api/portfolio/report` response. (3) Milestone-based reporting mode: new `milestone_id` query param on `GET /api/portfolio/report` — fetches milestone dates/epics from `milestones`+`milestone_epics`, overrides period dates, filters activities/projects/risks/issues to that milestone scope, uses milestone end-date month for bug snapshot; `portfolioMilestones` list + `milestoneInfo` added to response. `page.tsx`: mode toggle (Khoảng thời gian / Milestone), milestone dropdown, milestone info banner, all report generators use `data.periodStart`/`data.periodEnd`. |
